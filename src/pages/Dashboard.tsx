...
import { Building, ClipboardList, Wallet, Users as UsersIcon, TrendingUp, Settings2 } from 'lucide-react';
import {
...
const Dashboard = () => {
  const { properties, tasks, expenses, adminRecords, users } = useApp();
  const { isWidgetVisible, hasPropertyAccess, canConfigure } = usePermissions();

  const [customizerOpen, setCustomizerOpen] = useState(false);

  // Filter data to accessible properties
  const visibleProperties = properties.filter(p => hasPropertyAccess(p.id));
  const visibleTasks      = tasks.filter(t => hasPropertyAccess(t.propertyId));
  const visibleExpenses   = expenses.filter(e => hasPropertyAccess(e.propertyId));
  const activeUsers       = users.filter(u => u.status === 'active');

  const pendingTasks  = visibleTasks.filter(t => t.status !== 'Completado').length;
  ...
  const availableMonths = useMemo(() => {
    const unique = Array.from(new Set(adminRecords.map(r => r.period)));
    return unique;
  }, [adminRecords]);

  const [customMonth, setCustomMonth] = useState(
    () => availableMonths[availableMonths.length - 1] ?? 'Mayo 2026'
  );

  const adminData = useMemo(() => {
    if (dashPeriod === 'trimestre') return adminRecords;
    const target = dashPeriod === 'custom' ? customMonth : (availableMonths[availableMonths.length - 1] ?? '');
    return adminRecords.filter(r => r.period === target);
  }, [dashPeriod, customMonth, adminRecords, availableMonths]);
  ...
  const showFinancialSection =
    isWidgetVisible('chart_income') ||
    isWidgetVisible('chart_costs') ||
    isWidgetVisible('chart_margin');
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px' }}>
        {isWidgetVisible('kpi_properties') && (
          <div className="property-card" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '24px', fontWeight: 700 }}>{visibleProperties.length}</div>
            </div>
          </div>
        )}

        {isWidgetVisible('kpi_tasks') && (
          <div className="property-card" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        {isWidgetVisible('kpi_users') && (
          <div className="property-card" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ background: 'rgba(34, 34, 34, 0.1)', color: 'var(--text-main)', padding: '16px', borderRadius: '50%' }}>
              <UsersIcon size={28} />
            </div>
            <div>
              <div style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '4px' }}>Usuarios Activos</div>
              <div style={{ fontSize: '24px', fontWeight: 700 }}>{activeUsers.length}</div>
            </div>
          </div>
        )}
      </div>

      {/* ─── Control Administrativo — financial section ─── */}
      {showFinancialSection && (
        <div style={{ marginTop: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            {(isWidgetVisible('chart_costs') || isWidgetVisible('chart_margin')) && (
              <div className="property-card" style={{ padding: '20px' }}>
      ...
      {customizerOpen && (
        <DashboardCustomizer onClose={() => setCustomizerOpen(false)} />
      )}
    </div>
  );
};
